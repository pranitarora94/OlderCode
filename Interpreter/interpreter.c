...
int main(int argc, char* argv[])
{
	...
	lab_count = lab_no;
	setPcForMain();
    reg[14] = 262140;		// setting the stack pointer to 0xFFFFC (the end of the memory)
	reg[10] = 262140;
	while(pc < inst_count)
	{
...
void getReg3Add(char* inst, int i)
{
	...
	{ 
		...
		{
			if(imm > 524287)
				invalidInst();	//The largest positive number in 16 bit signed numbers is 0x7FFF = 524287
		}
		if(m == 2)
			imm *= 65536;
		isImm = 1;
	...
	else if(inst[i] == '-')		// if we have a negative decimal immediate
	{
		...
		imm = -imm;
		if(imm < -524288)
			invalidInst();	// Since the smallest negative number in 16 bit signed numbers is 0x8000 = -524288
		if(m == 1)
			imm += 65536;
		else if(m == 2)
			imm *= 65536;
...
void getLdSt(char* inst, int i)		// instruction is of the form rd, imm[rs1]
{
	...
	else if(isdigit(inst[i]))	// if we have a positive decimal immediate
	{
		...
		if(imm > 524287)
			invalidInst();		//The largest positive number in 20 bit signed numbers is 0x7FFF = 524287
	}
	...
	else if(inst[i] == '-')		// if we have a negative decimal immediate
	{
		...
		imm = -imm;
		if(imm < -524288)
			invalidInst();	// Since the smallest negative number in 20 bit signed numbers is 0x8000 = -524288
	}
